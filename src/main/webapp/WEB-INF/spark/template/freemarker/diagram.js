...
var Clickable = function (x, y, size, id){
 ...
 <#list rows as row>
     var asset = {type:"${row.type}", 
     <#if row.pointList?has_content>loc:[</#if><#list row.pointList as point>[${point[0]?c!}, ${point[1]?c!}],</#list><#if row.pointList?has_content>],</#if> id:"${row.id}", isOpen:"${row.isOpen?c}", phases:"${row.phases}" };
     assets.push(asset);
 </#list> 

for (var i = 0; i < activities.length; i++){
    a = activities[i];
    L.marker([a.geo[0][0], a.geo[0][1]]).bindPopup("<b>" + a.title + "</b><br />" + a.desc).addTo(points); 
};
...
centLat = (south + north) / 2
centLong = (west + east) / 2
...
var overlays = {"Jobs": points, "Grid": grid};
